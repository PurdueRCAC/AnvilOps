...
| ⭐  | `anvilops.env.cnameDomain`                                                                             | The domain name that custom domains are pointed to with CNAME DNS records. Populating this field with a domain name will create an Ingress at that hostname.                                                                                                           |                                                                                                 |
...
| ⭐  | `anvilops.apps.netpol.allowedIngressMatchLabels`                                                       | A list of label selectors that specify which namespaces are allowed to send ingress traffic to tenant apps. Apps in the same app group are always allowed. See notes below for important considerations.                                                               |                                                                                                 |
...
| ⭐  | `rancher.baseURL`                                                                                      | Base URL of Rancher, e.g. `https://composable.anvil.rcac.purdue.edu`.                                                                                                                                                                                                  |                                                                                                 |
...
|     | `buildkitd.affinity`                                                                                   |                                                                                                                                                                                                                                                                        |                                                                                                 |
|     | `buildkitd.netpol`                                                                                     | Set this to `null` to disable Kubernetes NetworkPolicies restricting ingress and egress of the Buildkit daemon and build jobs.                                                                                                                                         |                                                                                                 |
|     | `buildkitd.netpol.ingress`                                                                             | The list of ingress rules for a NetworkPolicy targeting the Buildkit daemon. Set this to `null` to allow all ingress. Set this to `[]` to deny all ingress.                                                                                                            | Allow ingress from build jobs. See `values.yaml`.                                               |
|     | `buildkitd.netpol.egress`                                                                              | The list of egress rules for a NetworkPolicy targeting the Buildkit daemon. Set this to `null` to allow egress anywhere. Set this to `[]` to deny all ingress.                                                                                                         | Allow egress to the outside internet and `kube-dns`. See `values.yaml`.                         |
|     | `buildkitd.netpol.buildctlIngress`                                                                     | The list of ingress rules for a NetworkPolicy targeting build jobs, which interact with the Buildkit daemon. Set this to `null` to allow all ingress. Set this to `[]` to deny all ingress.                                                                            | `[]`                                                                                            |
|     | `buildkitd.netpol.buildctlEgress`                                                                      | The list of egress rules for a NetworkPolicy targeting build jobs, which interact with the Buildkit daemon. Set this to `null` to allow all egress. Set this to `[]` to deny all egress.                                                                               | Allow egress to the internet, `kube-dns`, Buildkit, and the AnvilOps server. See `values.yaml`. |
...
The `allowedIngressMatchLabels` field accepts a list of label selectors identifying namespaces to allow traffic from. For example, this configuration allowlists traffic from `kube-system` and `ingress-nginx`.
