                      ...
                      {d.id === activeDeployment ? (
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-md">
                            Your domain points to this deployment. New
                            deployments become current once the app is built and
...
const RetryDeploymentPrompt = ({
  app,
  deployments,
  refetchDeployments,
}: {
  app: App;
  deployments: components["schemas"]["DeploymentEntry"][] | undefined;
  refetchDeployments: () => Promise<unknown>;
}) => {
  const { mutateAsync: updateApp, isPending } = api.useMutation(
    "put",
    "/app/{appId}",
  );
  ...
  if (deployments?.[0]?.status === "ERROR") {
    return (
      <div className="flex items-center justify-between rounded-md bg-orange-100 p-4">
        <p className="flex items-center gap-2">
          <TriangleAlert />
          The most recent deployment has failed.
        </p>
        <Button
          disabled={isPending}
          onClick={async () => {
