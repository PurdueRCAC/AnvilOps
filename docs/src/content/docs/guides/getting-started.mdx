...
If you would like to deploy an application on Anvil Composable, follow these instructions on `https://anvilops.rcac.purdue.edu`. If you would like to deploy on Geddes v2, follow these instructions on `anvilops.geddes.rcac.purdue.edu`.

## Setting Up Your Account

<Steps>

1. Sign in to AnvilOps with your ACCESS account.
...
- A **Rancher Project** to deploy your project inside.
  ...
  - In particular, a Project is a group of namespaces which can be configured with access controls and resource quotas.
...
If you are deploying a Git repository, you will need to specify the build configuration. If your repository already contains a Dockerfile, select the `Dockerfile` option and provide the path to the file. For instance, if the Dockerfile is in the root directory of your application, enter `Dockerfile`.
