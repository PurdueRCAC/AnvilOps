...
Navigate to the [create app](https://anvilops.rcac.purdue.edu/create-app) page.

### Organization

- **Organization**: All apps belong to an organization. Organizations connect apps to GitHub organizations and to the people who can access them on the AnvilOps dashboard. **This setting cannot be changed later.**

### Grouping Options

- **Group**: The name of a new or existing app group. Learn more about App Groups [here](/reference/app-groups).
...
- **Deployment Source**: Choose to deloy your app from a Git repository with automatic builds or directly from a OCI-compliant container image.
  ...
  - If you deploy from a Git repository, AnvilOps will watch for new commits and automatically rebuild and redeploy your application every time a new commit is pushed to the configured branch.
  - If you deploy from an OCI image, AnvilOps will bypass the build process and directly use the image you specify. Here are some [guidelines](https://kubernetes.io/docs/concepts/containers/images/#image-names) for allowed image names.
